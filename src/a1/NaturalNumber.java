...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
		...
		/* while loop that removes the last coefficient of each of the clones (in the form of an int), sums them with the carry, adds the 
		modulus of the result to the "sum" to be returned and updates the carry for further computations. The loop breaks when both addends
		of the addition no longer have terms to be added at which point the carry is added to the coefficients of the "sum" before it is 
		returned. */
		while (true) {
			
			int subsum = firstClone.coefficients.removeFirst() + secondClone.coefficients.removeFirst() + carry;
			sum.coefficients.addLast(subsum % this.base);
			carry = subsum/this.base;
		
			if(firstClone.coefficients.size() == 0) {
				if(carry !=0)
				sum.coefficients.addLast(carry);
				break;
			}
		}
		//remove leading zeros in case the inputs are padded with leading zeros
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		//make a clone since the operands are not to be changed
		NaturalNumber  firstClone  = this.clone();
		/*This for loop runs through each of the digits of the multiplier, singleDigit multiplies them with the multiplicand and adds the amount 
		 * of zeros that corresponds to the position of the selected digit in the multiplier before adding it to the "product" to be returned. */
		for (int i = 0; i < this.coefficients.size(); i++) {
			NaturalNumber multiply = firstClone.timesSingleDigit(multiplicand);
			for(int n = 0; n < i; n++) {
				multiply.coefficients.addFirst(0);
			}
			product= product.plus(multiply);
		}
		//Remove leading zeros in case inputs are padded with leading zeros
	...
	//multiplies a single digit of the caller by the 2nd, effectively a slowmultiply (didn't know if allowed to use *) 
	public NaturalNumber timesSingleDigit (NaturalNumber second) throws Exception {
		NaturalNumber prod = new NaturalNumber(0,this.base); 
		int add = this.coefficients.removeFirst();
		for (int i=0; i<add; i++) {
			prod=prod.plus(second);
		}
		return prod;
	}
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		int carry=0;
		//this loop makes sure both operands are of the same size by left padding the smaller one with zeros
		for(int i =0; i <this.coefficients.size(); i++) {
			while(first.coefficients.size() > secondclone.coefficients.size()) {
				secondclone.coefficients.addLast(0);
			}
			
			//current digit subtraction operation digits
			int cdigit1 = first.coefficients.get(i);
			int cdigit2 = secondclone.coefficients.get(i);
			//need to borrow
			if((cdigit1 - cdigit2 - carry <0)) {
				difference.coefficients.addLast(cdigit1+this.base-cdigit2-carry);
				carry=1;
			}else { //no need to borrow
				difference.coefficients.addLast(cdigit1-cdigit2-carry);
				carry=0;
			}			
		}		
	...
	public NaturalNumber divide( NaturalNumber  divisor ) throws Exception{
		...
		NaturalNumber dividebuffer = new NaturalNumber (this.base);
		/*This while loop continues while the remainder still has digits. It removes and appends one of the remainder digits to the "dividebuffer" 
		 * natural number at each iteration. The dividebuffer is then slowdivided (integer divided) by the divider and the result is added to the
		 * list of coefficients of the quotient. The divide buffer is then updated to be the difference of the itself and the multiplication of 
		 * the digit added to the quotient and the divider. The loop then re-iterates*/
		while(remainder.coefficients.size()!=0) {
			dividebuffer.coefficients.addFirst(remainder.coefficients.removeLast());
			dividebuffer.removeLeadingZeros();
			NaturalNumber temp = dividebuffer.slowDivide(divisor);
			quotient.coefficients.addFirst(temp.coefficients.peekFirst());
			dividebuffer=dividebuffer.minus(temp.times(divisor));
		}
		//remove leading zeros as a precaution against inputs with leading zeros
	...
	*/
	...
	//created a helper method from given code that removes leading zeros from an input NaturalNumber 
	public NaturalNumber removeLeadingZeros () {
		while ((this.coefficients.size()>1 & (this.coefficients.getLast().intValue()==0))) {
